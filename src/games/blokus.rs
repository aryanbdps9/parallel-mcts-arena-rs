    ...
    pub fn new(id: usize, shape: &[(i32, i32)]) -> Self {
        ...
        for _ in 0..2 {
            ...
            for _ in 0..4 {
                ...
                let mut normalized_shape: Vec<(i32, i32)> = current_shape
                    .iter()
                    .map(|p| (p.0 - min_r, p.1 - min_c))
                    .collect();
        ...
        Piece {
            id,
            transformations: {
                let mut sorted_transformations: Vec<Vec<(i32, i32)>> =
                    unique_transformations.into_iter().collect();
...
impl GameState for BlokusState {
        ...
        if self.consecutive_passes >= 3 {
            ...
            let has_valid_moves = available_pieces.iter().any(|piece| {
                piece.transformations.iter().any(|shape| {
                    (0..20).any(|r| (0..20).any(|c| self.is_valid_move(player_idx, shape, r, c)))
                })
            });
    ...
    fn get_winner(&self) -> Option<i32> {
        ...
        let mut scores = [0; 4];
        for i in 0..4 {
            scores[i] = self.player_pieces[i]
                .iter()
                .map(|p| p.transformations[0].len())
                .sum::<usize>() as i32;
        }
        ...
        for (dr, dc) in shape {
            ...
            for (nnr, nnc) in &neighbors {
                ...
                if ar >= 0
                    ...
                    && ac >= 0
                    && ac < 20
                    && self.board[ar as usize][ac as usize] == player_id
                {
                    return false;
                }
            }
