...
fn test_prune_kernel_pipeline_layout_validation() {
    ...
    let group4_layout = dummy_layout.clone();
    ...
    let shader = device.create_shader_module(include_wgsl!("shaders/mcts_othello.wgsl"));
    ...
    assert!(result.is_err(), "Pipeline creation should fail if group 3 is missing binding 1");
}
