...
use crate::games::blokus::{BlokusState, get_blokus_pieces};
use mcts::GameState;
use ratatui::prelude::*;
use ratatui::widgets::{
...
impl BlokusUIConfig {
    ...
    pub fn rotate_piece(&mut self, total_transformations: usize) {
        if total_transformations > 0 {
            self.selected_transformation_idx =
                (self.selected_transformation_idx + 1) % total_transformations;
        }
    }
    ...
    pub fn get_selected_piece_info(&self) -> Option<(usize, usize)> {
        self.selected_piece_idx
            .map(|piece_idx| (piece_idx, self.selected_transformation_idx))
    }
}
...
pub fn draw_blokus_piece_selection(f: &mut Frame, app: &App, area: Rect) {
    if let GameWrapper::Blokus(blokus_state) = &app.game_wrapper {
            ...
            if *is_expanded {
                ...
                for chunk_start in (0..total_pieces_to_show).step_by(pieces_per_row) {
                    ...
                    if !pieces_in_row.is_empty() {
                        ...
                        let separator_width = 1; // Each separator is 1 character: │
                        let content_width =
                            pieces_per_row * piece_width + (pieces_per_row - 1) * separator_width; // total content width
                        let total_grid_width = content_width + 2; // +2 for left and right borders
                        let available_width = inner_area.width as usize;
                        let padding = if available_width > total_grid_width {
                            (available_width - total_grid_width) / 2
        ...
        all_lines.push(Line::from(Span::styled(
            "1-9,0,a-k: Select piece  R: Rotate  X: Flip",
            Style::default().fg(Color::Gray),
        )));
        all_lines.push(Line::from(Span::styled(
            "Z: Toggle expand  +/-: Expand/Collapse all",
            Style::default().fg(Color::Gray),
        )));
        all_lines.push(Line::from(Span::styled(
            "Drag walls ◀▶ to resize panel",
        ...
        let visible_lines: Vec<Line> =
            if content_height > visible_height && scroll_offset < content_height {
                all_lines
                    .into_iter()
                    .skip(scroll_offset)
                    .take(visible_height)
                ...
                all_lines.into_iter().take(visible_height).collect()
            };
...
pub fn draw_debug_selected_piece(f: &mut Frame, app: &App, area: Rect) {
    let selected_info = if let Some((piece_idx, transformation_idx)) =
        app.blokus_ui_config.get_selected_piece_info()
    {
        format!(
            "Selected: Piece {} Transform {}",
            piece_idx, transformation_idx
        )
    } else {
...
pub fn draw_ghost_debug_info(f: &mut Frame, app: &App, area: Rect) {
    ...
    debug_lines.push(format!(
        "Cursor: ({}, {})",
        app.board_cursor.0, app.board_cursor.1
    ));
    ...
    if let GameWrapper::Blokus(state) = &app.game_wrapper {
        ...
        debug_lines.push(format!(
            "Player: {} | Available: {:?}",
            current_player,
            &available_pieces[..std::cmp::min(available_pieces.len(), 5)]
        ));
    }
    ...
    let paragraph = Paragraph::new(debug_text).block(
        Block::default()
            .borders(Borders::ALL)
            .title("Debug - Ghost Piece"),
    );
    f.render_widget(paragraph, area);
}
