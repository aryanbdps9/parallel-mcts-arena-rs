...
impl LayoutConfig {
    ...
    pub fn get_main_layout_dynamic(
        &self,
        area: Rect,
        board_height: usize,
        board_width: usize,
        ...
        let required_board_height =
            Self::calculate_required_board_height(board_height, board_width, game_type);

        // Use the larger of: required height or user's dragged preference (but capped)
        let board_height_from_percent =
            (area.height as f32 * self.board_height_percent as f32 / 100.0) as u16;
        let actual_board_height = required_board_height
            .max(board_height_from_percent)
            .min(area.height - 10); // Leave at least 10 rows for bottom
    ...
    pub fn get_drag_boundaries(&self, terminal_size: Rect) -> Vec<(DragBoundary, u16, u16)> {
        ...
        let board_height =
            (terminal_size.height as f32 * self.board_height_percent as f32 / 100.0) as u16;
    ...
    pub fn handle_drag(&mut self, boundary: DragBoundary, delta: i16, _terminal_size: Rect) {
        match boundary {
            DragBoundary::BoardBottom => {
                let new_percent =
                    ((self.board_height_percent as i16 + delta).max(20).min(80)) as u8;
                self.board_height_percent = new_percent;
            }
            DragBoundary::BlokusPieceSelectionLeft => {
                let new_width = ((self.blokus_piece_selection_width as i16 + delta)
                    .max(30)
                    .min(80)) as u16;
    ...
    pub fn calculate_required_board_height(
        board_height: usize,
        board_width: usize,
        game_type: &str,
    ) -> u16 {
    ...
    pub fn calculate_required_board_width(
        board_height: usize,
        board_width: usize,
        game_type: &str,
    ) -> u16 {
