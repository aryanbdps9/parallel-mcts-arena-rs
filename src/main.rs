...
pub mod components;     // Modular UI components and event system
    ...
    if let Some(game_name) = &args.game {
        match game_name.as_str().to_lowercase().as_str() {
            ...
            "connect4" => {
                ...
                if args.board_size == 15 { // User didn't specify custom size
                    args.board_size = 7; // Standard Connect4 width
                }
                if args.line_size == 5 { // User didn't specify custom line size
