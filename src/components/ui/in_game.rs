...
use crate::components::blokus::{
    BlokusBoardComponent, BlokusGameStatsComponent, BlokusInstructionPanelComponent,
    ImprovedBlokusPieceSelectorComponent,
};
...
///     ├── ImprovedBlokusPieceSelectorComponent (advanced piece selection)
...
pub struct InGameComponent {
    ...
    /// Blokus piece selector component with advanced features
    /// Includes piece rotation, ghost preview, scrolling, and smart layout
...
impl InGameComponent {
    ...
    /// 2. ImprovedBlokusPieceSelectorComponent - Advanced piece selection with all features
    /// 3. BlokusGameStatsComponent - Statistics and scoring
    /// 4. BlokusInstructionPanelComponent - Help and instructions
    fn ensure_blokus_components(&mut self) {
        ...
        // Initialize the advanced piece selector with all features
    ...
    fn render_stats_history_tabs(
        ...
        match app.active_tab {
            crate::app::ActiveTab::Debug => {
                self.render_debug_stats_content(frame, inner_area, app)?
            }
            crate::app::ActiveTab::History => {
                self.render_move_history_content(frame, inner_area, app)?
            }
        }
    ...
    fn render_move_history_content(
        &self,
        frame: &mut Frame,
        area: Rect,
        app: &App,
    ...
    fn render_blokus_game_view(
        ...
        let (board_area, piece_area, _player_area) =
        ...
        // Render piece selection panel using the improved component
    ...
    #[allow(dead_code)]
    fn render_blokus_board_with_ghost(
        &self,
        frame: &mut Frame,
        area: Rect,
        app: &App,
        state: &crate::games::blokus::BlokusState,
        ...
        let last_move_positions: HashSet<(usize, usize)> = state
            .get_last_move()
        ...
        for (r, row) in board.iter().enumerate() {
            ...
            for (c, &cell) in row.iter().enumerate() {
                ...
                let (symbol, style) = if is_ghost {
                    ...
                    let is_legal = if let Some((piece_id, transformation, cursor_row, cursor_col)) =
                        selected_piece
                    {
                        use crate::games::blokus::BlokusMove;
                        let test_move =
                            BlokusMove(piece_id, transformation, cursor_row, cursor_col);
    ...
    #[allow(dead_code)]
    fn get_ghost_piece_positions(
        &self,
        _state: &crate::games::blokus::BlokusState,
        piece_id: usize,
    ...
    #[allow(dead_code)]
    fn render_blokus_piece_selection(
        &self,
        frame: &mut Frame,
        area: Rect,
        app: &App,
            ...
            for player in 1..=4 {
                ...
                let header_text = if is_current {
                    format!(
                        "{} ► {} ({} pieces) ◄",
                        expand_indicator,
                        player_names[(player - 1) as usize],
                        available_count
                    )
                } else {
                    format!(
                ...
                if *is_expanded {
                    ...
                    if is_current && total_pieces_to_show > 0 {
                        ...
                        let content_width =
                            pieces_per_row * 7 + (pieces_per_row - 1) * separator_width; // 7*5 + 4*1 = 39
                    ...
                    for chunk_start in (0..total_pieces_to_show).step_by(pieces_per_row) {
                        ...
                        for display_idx in chunk_start..chunk_end {
                            ...
                            let is_selected = is_current
                                && app.blokus_ui_config.selected_piece_idx == Some(piece_idx);
                        ...
                        if !pieces_in_row.is_empty() {
                            ...
                            let separator_width = 1; // Each separator is 1 character: │
                            let content_width = pieces_per_row * piece_width
                                + (pieces_per_row - 1) * separator_width; // total content width
                            let total_grid_width = content_width + 2; // +2 for left and right borders
                            let available_width = inner_area.width as usize;
                            let padding = if available_width > total_grid_width {
                                (available_width - total_grid_width) / 2
                            ...
                            key_line_spans.push(Span::styled(
                                left_padding + "│",
                                Style::default().fg(Color::DarkGray),
                            ));
                            ...
                            for (i, (piece_name, _, style)) in pieces_in_row.iter().enumerate() {
                                let padded_name =
                                    format!("{:^width$}", piece_name, width = piece_width);
                            ...
                            for line_idx in 0..max_height {
                                ...
                                // Add left padding and border
                                let left_padding = " ".repeat(padding);
                                shape_line_spans.push(Span::styled(
                                    left_padding + "│",
                                    Style::default().fg(Color::DarkGray),
                                ));

                                for (i, (_, piece_visual_lines, style)) in
                                    pieces_in_row.iter().enumerate()
                                {
                                ...
                                for i in pieces_in_row.len()..pieces_per_row {
                                    let empty_cell = " ".repeat(piece_width);
                                    shape_line_spans
                                        .push(Span::styled(empty_cell, Style::default()));
                                    if i < pieces_per_row - 1 {
                                        shape_line_spans.push(Span::styled(
                                            "│",
                                            Style::default().fg(Color::DarkGray),
                            ...
                            if chunk_start + pieces_per_row < total_pieces_to_show {
                                let left_padding = " ".repeat(padding);
                                let row_separator =
                                    "├".to_string() + &"─".repeat(content_width) + "┤";
                                all_lines.push(Line::from(Span::styled(
                                    left_padding + &row_separator,
                                    Style::default().fg(Color::DarkGray),
                                )));
                            }
                        }
    ...
    #[allow(dead_code)]
    fn render_blokus_player_status(
        &self,
        frame: &mut Frame,
        area: Rect,
        app: &App,
...
impl Component for InGameComponent {
    ...
    fn handle_event(&mut self, event: &ComponentEvent, app: &mut App) -> EventResult {
        ...
        if matches!(app.game_wrapper, GameWrapper::Blokus(_)) {
            ...
            // Try to handle the event with the improved Blokus piece selector
            if let Some(ref mut improved_piece_selector) = self.improved_blokus_piece_selector {
        ...
        match event {
            ...
            ComponentEvent::Input(InputEvent::MouseClick { x, y, button }) => {
                ...
                if self.is_current_player_human(app)
                    || matches!(app.game_wrapper, GameWrapper::Blokus(_))
                {
                    match button {
                        1 => {
                            // Left click
            ...
            ComponentEvent::Input(InputEvent::MouseScroll { x: _, y: _, up: _ }) => {
                ...
                if matches!(app.game_wrapper, GameWrapper::Blokus(_)) {
                    if let Some(ref mut improved_piece_selector) =
                        self.improved_blokus_piece_selector
                    {
...
impl InGameComponent {
    ...
    fn handle_blokus_mouse_click_ai_turn(&self, app: &mut App, col: u16, row: u16) {
            ...
            && row < piece_area.y + piece_area.height
        {
            ...
            self.handle_blokus_piece_expand_collapse_only(
                app,
                col - piece_area.x,
                row - piece_area.y,
            );
    ...
    fn handle_blokus_click(&self, app: &mut App, col: u16, row: u16) {
            ...
            && row < piece_area.y + piece_area.height
        {
            ...
            self.handle_blokus_piece_selection_click(
                app,
                col - piece_area.x,
                row - piece_area.y,
                piece_area.width,
    ...
    fn get_blokus_layout_areas(&self, terminal_size: Rect) -> (Rect, Rect, Rect) {
        ...
        let piece_area = Rect::new(
            main_area.x + board_width,
            main_area.y,
            piece_width,
            main_area.height,
        );
        let player_area = Rect::new(
            main_area.x + board_width + piece_width,
            main_area.y,
            player_width,
            main_area.height,
        );
    ...
    fn handle_blokus_piece_selection_click(
        &self,
        app: &mut App,
        col: u16,
        row: u16,
            ...
            for player in 1..=4 {
                ...
                if *is_expanded {
                    if is_current {
                        ...
                        for chunk_start in (0..total_pieces_to_show).step_by(pieces_per_row) {
                            let chunk_end =
                                (chunk_start + pieces_per_row).min(total_pieces_to_show);
                            ...
                            for display_idx in chunk_start..chunk_end {
                                    ...
                                    && !pieces[display_idx].transformations.is_empty()
                                {
                                    ...
                                    let piece_visual_lines =
                                        self.create_visual_piece_shape(piece_shape);
