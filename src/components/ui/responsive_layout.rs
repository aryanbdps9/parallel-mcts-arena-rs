...
impl ResponsiveLayoutComponent {
    ...
    fn calculate_constraints(&self, available_space: u16) -> Vec<Constraint> {
        match self.layout_type {
            ...
            ResponsiveLayoutType::FixedRatio => self.calculate_fixed_ratio_constraints(available_space),
            ResponsiveLayoutType::ContentDriven => self.calculate_content_driven_constraints(available_space),
        }
    }
    ...
    fn calculate_fixed_ratio_constraints(&self, _available_space: u16) -> Vec<Constraint> {
        ...
        self.preferred_sizes.iter().map(|&size| {
...
impl Component for ResponsiveLayoutComponent {
    ...
    fn render(&mut self, _frame: &mut ratatui::Frame, area: Rect, _app: &crate::app::App) -> ComponentResult<()> {
        self.set_area(area);
