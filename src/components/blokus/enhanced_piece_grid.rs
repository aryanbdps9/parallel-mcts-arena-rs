...
impl EnhancedPieceGridComponent {
    ...
    fn update_responsive_layout(&mut self, available_width: u16) {
        ...
        if usable_width > 0 {
            let pieces_that_fit = ((usable_width as usize + separator_width)
    ...
    pub fn calculate_height(&self) -> u16 {
        ...
        let rows =
            ((pieces_to_show + self.config.pieces_per_row - 1) / self.config.pieces_per_row).max(1);
        let row_height = self.config.piece_height + (if self.config.show_labels { 1 } else { 0 });
...
impl Component for EnhancedPieceGridComponent {
    ...
    fn render(&mut self, frame: &mut Frame, area: Rect, app: &App) -> ComponentResult<()> {
        ...
        let rows = ((pieces_to_display.len() + self.config.pieces_per_row - 1)
            / self.config.pieces_per_row)
            .max(1);
        ...
        for row in 0..rows {
            ...
            let pieces_in_row: Vec<usize> = pieces_to_display[chunk_start..chunk_end]
                .iter()
                .cloned()
                .collect();
            ...
            let max_height = pieces_in_row_data
                .iter()
            ...
            if self.config.show_labels {
                ...
                for (i, (piece_label, _, style)) in pieces_in_row_data.iter().enumerate() {
                    let padded_label =
                        format!("{:^width$}", piece_label, width = self.config.piece_width);
            ...
            if row < rows - 1 {
                let separator_width = self.config.pieces_per_row * self.config.piece_width
                    + (self.config.pieces_per_row - 1);
