...
impl ResponsivePieceGridComponent {
    ...
    fn update_layout(&mut self) {
        ...
        let (pieces_per_row, total_rows) =
            self.layout_calculator.calculate_optimal_layout(piece_count);
    ...
    fn update_responsive_layout(&mut self, available_width: u16) {
        ...
        let border_width = if self.layout_calculator.get_config().show_borders {
            2
        } else {
            0
        };
        ...
        if usable_width > 0 {
            ...
            let max_pieces_that_fit = ((usable_width as usize + separator_width)
                / (self.layout_calculator.get_config().piece_width + separator_width))
                .max(1);

            // Update layout calculator with new constraints
            self.layout_calculator
                .update_width_constraints(max_pieces_that_fit);
    ...
    pub fn calculate_height(&self) -> u16 {
        ...
        let separator_height = if self.total_rows > 1 {
            self.total_rows as u16 - 1
        } else {
            0
        };
        ...
        for piece_idx in pieces_in_row {
            if *piece_idx < pieces.len() {
                ...
                let style = if is_selected {
                    ...
                    Style::default()
                        .fg(self.get_config().player_color)
                        .add_modifier(Modifier::BOLD)
                } else {
                    Style::default()
                        .fg(Color::DarkGray)
                ...
                let content = if col < pieces_data.len() {
                    ...
                    if line_index == 0 && self.get_config().show_labels {
                        ...
                        format!(
                            "{:^width$}",
                            label_text,
                            width = self.get_config().piece_width
                        )
                    } else {
                        ...
                        if visual_line_index < piece_visual_lines.len() {
                            ...
                            if current_width < self.get_config().piece_width {
                                ...
                                piece_line
                                    .chars()
                                    .take(self.get_config().piece_width)
                                    .collect()
                            } else {
...
impl Component for ResponsivePieceGridComponent {
    ...
    fn render(&mut self, frame: &mut Frame, area: Rect, app: &App) -> ComponentResult<()> {
        ...
        if self.available_pieces.is_empty() {
            let no_pieces =
                Paragraph::new("No pieces available").style(Style::default().fg(Color::DarkGray));
        ...
        for row in 0..self.total_rows {
            ...
            let pieces_in_row: Vec<usize> = self.available_pieces[chunk_start..chunk_end]
                .iter()
                .cloned()
                .collect();
