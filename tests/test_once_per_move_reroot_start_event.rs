//! Integration test: Only one BATCH_START event per kernel dispatch
...
#[test]
#[ignore = "BATCH_START event emission was disabled in shader"]
fn test_batch_start_event() {
    ...
    for idx in start_idx..write_head {
        ...
        if event.event_type == 10 { // BATCH_START
            batch_start_count += 1;
            println!("Found BATCH_START event at idx {}", idx);
        }
    }
    drop(buffer_data);
    urgent_event_staging.unmap();
    
    assert_eq!(batch_start_count, 1, "Expected exactly one BATCH_START event, got {}", batch_start_count);
}
