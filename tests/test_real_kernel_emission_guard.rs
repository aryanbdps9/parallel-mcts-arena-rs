//! Test harness to dispatch the real MCTS kernel and directly inspect the emission guard atomic and urgent events
...
mod tests {
    ...
    fn test_real_kernel_emission_guard() {
        ...
        // Set up a minimal board and legal moves
        let board = [0; 64];
        let root_player = 1;
        let legal_moves = &[(0, 0)];
        ...
        // Assert atomic is zero (all threads decremented)
        assert_eq!(atomic_val, 0, "Real kernel emission guard: atomic not decremented to zero!");

        // TODO: Optionally, read urgent event buffer and check only one REROOT_END event was emitted
    }
}
