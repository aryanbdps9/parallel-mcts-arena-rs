//! Test that a compute kernel can write to a storage buffer bound at group(5), binding(0)
...
mod tests {
    ...
    #[test]
    fn test_group5_binding0_storage_buffer_write() {
        // Setup wgpu device/queue using GpuContext directly
        let config = GpuConfig::default();
        let context = GpuContext::new(&config).expect("Failed to create GpuContext");
        let device = context.device();
        let queue = context.queue();

        // Create a storage buffer (4 bytes for u32)
        let buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Test Storage Buffer"),
            contents: &[0, 0, 0, 0],
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC | wgpu::BufferUsages::COPY_DST,
        });

        // WGSL kernel: writes 0xDEADBEEF (u32) to the buffer
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Test Write Kernel"),
            source: wgpu::ShaderSource::Wgsl(r#"
@group(5) @binding(0) var<storage, read_write> outbuf: array<u32, 1>;
@compute @workgroup_size(1)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    outbuf[0] = 0xDEADBEEFu;
}
"#.into()),
        });
        ...
        assert_eq!(val, 0xDEADBEEF, "Kernel did not write expected value to buffer!");
    }
}
