//! Test that the main kernel emission guard fails if the atomic is not decremented, and passes if it is.
...
mod tests {
    ...
    #[test]
    fn test_main_kernel_emission_guard_bug() {
        ...
        // Create a buffer for the atomic (initialized to num_threads)
        let buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Test Emission Guard Buffer"),
            contents: &num_threads.to_le_bytes(),
        ...
        // WGSL kernel: intentionally skip atomicSub to simulate the bug
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Test Main Kernel Emission Guard Bug"),
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    // let prev = atomicSub(&global_atomic, 1u); // BUG: not called
    let prev = 0u; // Simulate all threads seeing prev == 0
    if (prev == 1u) {
        outbuf[0] = outbuf[0] + 1u;
    }
}
"#.into()),
        });
        ...
        assert_eq!(val, 0, "Emission guard bug: event was emitted when it should not have been!");
    }
}
