//! Test real kernel emission guard with a large number of legal moves
#[cfg(test)]
mod tests {
    use mcts::{GpuOthelloMcts, GpuConfig, GpuContext};
    use std::sync::Arc;
    #[test]
    fn test_real_kernel_emission_guard_many_legal_moves() {
        ...
        let board = [0; 64];
        let root_player = 1;
        let legal_moves: Vec<_> = (0..8).flat_map(|x| (0..8).map(move |y| (x, y))).collect();
        ...
        assert_eq!(atomic_val, 0, "Real kernel emission guard (many legal moves): atomic not decremented to zero!");
    }
}
